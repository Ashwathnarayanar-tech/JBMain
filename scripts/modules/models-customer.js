define(['modules/backbone-mozu', 'underscore', 'modules/models-address', 'modules/models-orders', 
            ...
            "address.addressType": {
                fn: function (value) { 
                    if(value === "Nan" || typeof value === "undefined")   
                        return Hypr.getThemeSetting("addressType");
                }
            }
        },
    ...
    Customer = Backbone.MozuModel.extend({
        ...
        addStoreCredit: function (id) {
            var temp = this.apiAddStoreCredit(id);
            temp.then(function(){
                return temp;
            }).catch(function(e){
                return null;
    ...
    EditableCustomer = Customer.extend({
        ...
        saveCard: function () {  
            if (!this.validate('editingCard')) {
                var self = this,saveContactOp,editingCard = this.get('editingCard').toJSON(),
                doSaveCard = function () {
                ...
                if (!editingCard.contactId || editingCard.contactId === "new") {
                    ...
                    if (saveContactOp && !saveContactOp.isError){ return saveContactOp.then(doSaveCard);}else if(saveContactOp && saveContactOp.isError){return saveContactOp;}
                } else {
                    return doSaveCard();  
                }
            }else{
                var errors = this.validate('editingCard');
                var result = {};
                result.isError = true;
                result.errors = errors;
                if(!errors['editingCard.contactId']){
                    return result;  
                }
            }
        },
