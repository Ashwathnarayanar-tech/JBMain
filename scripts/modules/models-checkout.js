    ...
    function($, _, Hypr, Backbone, api, CustomerModels, PaymentMethods, HyprLiveContext) {
        ...
        var CheckoutStep = Backbone.MozuModel.extend({
            ...
            calculateStepStatus: function () {
                ...
                chooseAutomatically: function(idx) {
                    if (idx !== -1) {
                        var addr = this.get('address');
                ...
                next: function() {
                    ...
                    this.set('address.countryCode', 'US');
                    this.set('address.stateOrProvince', state);
                ...
                var completeStep = function () {
                    ...
                    order.apiModel.getShippingMethodsFromContact().then(function (methods) {
                        ...
                        setTimeout(function() {
                            window.scrollTo(0, $('#step-shipping-method').offset().top);
                        }, 80);
            ...
            FulfillmentInfo = CheckoutStep.extend({
                ...
                validation: {
                    shippingMethodCode: {
                        required: true,
                        msg: Hypr.getThemeSetting('chooseShippingMethod')
                    }
                },
                    ...
                    order.apiModel.update({ fulfillmentInfo: me.toJSON() })
                        .then(function (o) {
                            ...
                            if (billingInfo) {
                                ...
                                // billingInfo.setSavedPaymentMethod(billingInfo.get('card.id'));
                ...
                syncPaymentMethod: function(me, newId) {
                    ...
                    me.set('usingSavedCard', false);
                       ...
                       // me.get('billingContact').clear();
                        ...
                        if(me.get('card').get('cvv')){  
                            me.get('card').attributes.cvv = ''; 
                        }
                    }
                },
        ...
        var ShopperNotes = Backbone.MozuModel.extend();
             var pattern1 = /^.*(?=.{6,})(?=.*\d)(?=.*[@#$%^&+=_()*-]).*$/;
             var pattern2 =/^.*(?=.{6,})(?=.*[0-9])([a-zA-Z0-9\s]+)$/;
            var checkoutPageValidation = {
                'emailAddress': {
                    fn: function(value) {
                        if (this.attributes.createAccount && (!value || !value.match(Backbone.Validation.patterns.email))) {
                            $('#new-account-email').attr('aria-invalid',true).focus();
                ...
                'password': {
                    fn: function(value) {
                        if (this.attributes.createAccount && !value){
                            $('#newaccountpassword').attr('aria-invalid',true).focus();
                            return Hypr.getLabel('passwordMissing');
                        }
                        else if (this.attributes.createAccount && value ){
                            if(value.match(pattern2) !== null || value.match(pattern1) !== null){
                                return false;
                            }else{
                                $('#newaccountpassword').attr('aria-invalid',true).focus();
                                return Hypr.getLabel('passwordMissing');
                ...
                'confirmPassword': {
                    ...
                    fn: function(value) {  
                    //     if(this.attributes.createAccount && !value){
                    //       return  Hypr.getThemeSetting('passwordsDoNotMatch');    
                    //     }
                    //   else
                    if(this.get('password')){
                        if(this.get('password').match(pattern1) || this.get('password').match(pattern2)){
                            if (this.attributes.createAccount && !value && (value !== this.get('password'))){
                                    $('#confirmpassword').attr('aria-invalid',true).focus();
                                    return Hypr.getLabel('passwordsDoNotMatch');
                                }
                                else if ((value.match(pattern1) !== null || value.match(pattern2) !== null )){
                                     if(value !== this.get('password')){
                                        $('#confirmpassword').attr('aria-invalid',true).focus();
                                        return Hypr.getThemeSetting('passwordsDoNotMatchforaccount'); 
                                      }else{     
                                        return false;
                                      }
                                }else{
        ...
        var CheckoutPage = Backbone.MozuModel.extend({
            ...
            onCheckoutError: function(error) {
                ...
                if (error.message && error.message.indexOf("does not have enough stock to create this reservation and backorders are not enabled. Reservations are not allowed at this time.") > 0) {
