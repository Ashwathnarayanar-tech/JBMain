    ...
    window.paypalCheckoutReady = function() {
      ...
      var siteContext = hyprlivecontext.locals.siteContext,
       ...
       var merchantAccountId = _.findWhere(externalPayment.credentials, {"apiName" : "merchantAccountId"}),
          ...
          id = CartModels.Cart.fromCurrent().id || window.order!==undefined?window.order.id:"",
            ...
            click: function(event) {
               ...
               /* if(isSafari && device < 1025){
                     $.colorbox({
                        ...
                        scrolling: false,
                        fadeOut: 500,
                        html:  "<div style='padding: 20px; text-align: center;'><p>We're sorry, but PayPal is not an available option right now.</p></div>", //"/resources/intl/geolocate.html",
                        overlayClose: true,
                        onComplete: function(){
                            $('#cboxClose').show();
                            $('#cboxLoadedContent').css({ background: "#ffffff" });
