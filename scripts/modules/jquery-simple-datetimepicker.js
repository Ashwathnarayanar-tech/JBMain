 ...
 */
 ...
 (function($) {
    var lang = {
        ...
        ua: {
            days: ['Неділя','Понеділок','Вівторок','Cереда','Четвер','Пятниця','Субота'],
    ...
    **/
    ...
    PickerHandler.prototype._relocate = function(){
        ...
        if ($input !== null && $picker.data('isInline') === false) { // Float mode
            ...
            if(parseInt($(window).height(), 10) <=  ($input.offset().top - $(document).scrollTop() + input_outer_height + picker_outer_height) ){
    ...
    **/
    ...
    var normalizeYear = function (year) {
        if (year < 99) { // change year for 4 digits
            var date = new Date();
            return parseInt(year, 10) + parseInt(date.getFullYear().toString().substr(0, 2) + '00', 10);
        }
        return year;
    };
    ...
    var init = function($obj, opt) {
        ...
        opt.minTime[0]=parseInt(opt.minTime[0], 10);
